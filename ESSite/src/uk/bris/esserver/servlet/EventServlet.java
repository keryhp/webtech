	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user != null){
			...
			if(messages.isEmpty()){
				...
				session.setAttribute("cmessage", "Event added successfully!");
				request.getRequestDispatcher("/index.jsp").forward(request, response);			
			}else{
				session.setAttribute("cmessage", "Event creation failed! " + messages.values().toString());			
				request.getRequestDispatcher("/index.jsp").forward(request, response);			
			}
		}else {
			session.setAttribute("cmessage", "Please login to post an event!");			
			request.getRequestDispatcher("/index.jsp").forward(request, response);						
		}
